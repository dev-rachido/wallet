...
const deleteItem = async (id) => {
  ...
  return (
    <div className="  m-2  h-screen bg-blend-color-dodge opacity-90 pt-10 ">
      <EmblaCarousel />
      <h1 className="text-center text-3xl border-b-2 mb-16 w-1/4 m-auto p-5 mt-8 border-slate-500 text-cyan-800 font-semibold">
        Lijst
      </h1>
      <div className="h-100   ">
        <div className="md:flex extrastijl   border rounded-lg w-3/4 m-auto bg-cyan-800 space-between p-3">

          <form className=" p-1 m-auto  text-cyan-950  w-full mt-10  h-4/6  ">
          ...
          <div className="border rounded-lg p-2  m-auto font-semibold w-4/6 bg-slate-300  ">
            <ul className="pt-4 text-slate-600 text-left text-xs lg:gap-x-2   md:text-sm  lg:grid-cols-2 lg:grid lg:text-lg">
              {products.map((product) => (
                <li
                  className="uppercase min-w-fit text-xs flex m-1 w-full  bg-slate-100 rounded-md p-1"
                  key={product.id}
                >
                  <div className=" lg:w-full flex justify-between m-auto ">
        ...
        <div className="extrastijl border-4 w-9/12 mt-8  py-4 font-bold text-3xl rounded  bg-slate-200 m-auto px-10 flex flex-row justify-between text-center  ">
          <h1>Totaal</h1>
          {products.length < 1 ? (
            ""
          ) : (
